...
const BLUES=['#60A5FA','#7DB3FF','#4F8EF7','#93C5FD','#5B9DF5','#3B82F6','#38BDF8','#7AA2F7','#8AB4F8','#4C7DF0'];
// jewel actives
const JEWELS={Sapphire:'#1D4ED8',Emerald:'#10B981',Ruby:'#E11D48',Amethyst:'#9333EA',Gold:'#F5B301',Crimson:'#DC2626',Turquoise:'#14B8A6',Topaz:'#F97316'};
...
const STY=[['Royal',82,68],['Retro',58,72],['Neon',95,66],['Pastel',62,80]];
const POOL0=[];for(const h of [0,25,45,60,90,130,155,175,270,290,310,335])for(const [s,l] of [[75,72],[88,66]])POOL0.push(hex(h,s,l));
const POOL=POOL0.filter(okText);
const INACT=['#94A3B8','#A39FB5'];
...
for(const [mn,mh] of MAIN)for(const [sn,S,L] of STY){
  ...
  for(const [jn,active] of Object.entries(JEWELS)){ let best=null;
    ...
    if(dE(session,active)<38)continue;
    for(const docker of BLUES){ if(!okText(docker)||dE(docker,active)<32||dE(docker,session)<30)continue;
      for(const inact of INACT){
        const base=[session,active,docker,inact];
        // choose date/time greedily-best pair
        let bd=null;
        for(let i=0;i<POOL.length;i++){const d=POOL[i];const m1=Math.min(...base.map(b=>dE(b,d)));if(m1<30)continue;
          for(let j=i+1;j<POOL.length;j++){const t=POOL[j];const m2=Math.min(...base.map(b=>dE(b,t)),dE(d,t));if(m2<30)continue;
...
const ORDER=['Sapphire','Emerald','Ruby','Amethyst','Gold','Crimson','Turquoise','Topaz'];
out.sort((a,b)=>b[7]-a[7]);
const seen=new Set();for(let i=0;i<out.length;i++){const k=out[i][0].split(' ')[0]+out[i][8];if(out[i][7]<32||seen.has(k)){out.splice(i--,1)}else seen.add(k)}
...
if(process.argv[2]==='write'){const fs=require('fs');let s=fs.readFileSync('index.html','utf8');s=s.replace(/const P=\[[\s\S]*?\];\n/,'const P='+JSON.stringify(out)+';\n');fs.writeFileSync('index.html',s)}
